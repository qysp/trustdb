...
function applyQuery(queryObj, doc) {
  ...
  const resultOperation = chooseOperation(queryObj.$op);
  const negateResult = useNegation(queryObj.$op);
  delete queryObj.$op;
  
  const endResult = Object.entries(queryObj)[resultOperation](([ key, query ]) => {
    if (isPlainObject(query)) {
      const queryOperation = chooseOperation(query.$op);
      const negateQuery = useNegation(query.$op);
      delete query.$op;
