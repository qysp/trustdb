 ...
 * 
 * @param {object} queryObj queryObj to parse and apply on the document
...
function applyQuery(queryObj, doc) {
  // Create a copy to not alter the original queryObj.
  ...
  const endResult = Object.entries(_queryObj)[resultOperation](([ key, query ]) => {
    if (isPlainObject(query)) {
      // Since the spread syntax only performs a shallow clone, we have to clone it again to not alter the original queryObj.
      const _query = { ...query };
...
/**
 * Check if the query has an unknown query function property (aka. check).
 * 
  ...
  return Object.values(queryObj).some(val => {
    if (isPlainObject(val)) {
      return Object.keys(val).some(fn =>
        !hasOwnProperty(checks, fn) && fn !== '$op'
      );
    }
  });
}
