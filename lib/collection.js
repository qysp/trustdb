...
class Collection extends EventEmitter {
   ...
   * @param {boolean} resultsInstance whether the matching documents should be returned as a Results instance (for sorting)
   * @fires Collection#find after successfully filtering documents
   * @returns {Promise<object[] | Results | Error>} a Promise with the matching documents
   * @example
   ...
   */
  find(filter, resultsInstance = false) {
    return new Promise(async (resolve, reject) => {
      ...
      const results = resultsInstance === true
        ? new Results(filteredDocs)
        : filteredDocs;
