...
class Collection extends EzeventEmitter {
   ...
   * @fires Collection#inserted after successfully inserting a document
  ...
  insert(...docs) {
    return new Promise(async (resolve, reject) => {
      ...
      await this.emit('inserted', this.documents);
   ...
   * @param {boolean} [emitEvent=true] whether to emit the *inserted* event
   * @fires Collection#inserted after successfully inserting a document if `emitEvent` is `true`
   * @returns {Promise<number | Error>} number of documents in the collection
   */
