...
class Collection extends EventEmitter {
  ...
  find(query, op = 'and') {
    return new Promise(async (resolve, reject) => {
      ...
      if (!checks.isPlainObject(query)) {
        reject(new TypeError('Query must be a plain object'));
      }
      if (checks.hasUnknownCheck(query)) {
   ...
   * 
   * @param {function} filterFn custom filter function which will be applied upon each document
   * @fires Collection#find after successfully filtering documents
   * @returns {Promise<object[] | Error>} a Promise with the matching documents
   * @example
  ...
  findWhere(filterFn) {
    return new Promise(async (resolve, reject) => {
      ...
      const results = this.documents.filter(doc => filterFn(doc));
   ...
   * 
   * @param {function} filterFn custom filter function which will be applied upon each document
   * @param {function} updateFn custom update function which will be applied upon each filtered document
   ...
   * @example
   * // Update all documents where the value of the
   * // property price is greater than or equal to 50
   * // and set it to 49.99.
   * const updatedDocs = await collection.updateWhere(
   *   doc => doc.price >= 50,
   *   doc => doc.price = 49.99
   * );
   */
  updateWhere(filterFn, updateFn) {
    return new Promise(async (resolve, reject) => {
      if (typeof filterFn !== 'function' || typeof updateFn !== 'function') {
        reject(new TypeError('Filter/Updater must be be functions'));
      }
