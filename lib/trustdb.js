...
class TrustDB extends EventEmitter {
   ...
   * @returns {Collection} created collection or exisiting collection with the same name
   */
  createCollection(name) {
    if (typeof name !== 'string' || name === '') {
      throw new TypeError('Parameter `name` must be a string and at least one character long');
    }
  ...
  addCollection(collection) {
    if (!(collection instanceof Collection)) {
      throw new TypeError('Parameter `collection` must be an instance of Collection');
    }
