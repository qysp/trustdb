...
const { isPlainObject } = require('./checks');
...
class TrustDB extends EventEmitter {
  ...
  connect(filepath, settings = {}) {
    return new Promise(async (resolve, reject) => {
      if (typeof filepath !== 'string' || filepath === '') {
        reject(new Error('Filepath must be a string and at least one character long'));
      }
      ...
      this.configureSettings(this._settings);
      resolve();
    });
  }
  ...
  saveDatabase() {
    return new Promise(async (resolve, reject) => {
      ...
      writeFile(this._filepath, exportedDb)
        .then(resolve)
        .catch(reject);
    });
  }
  ...
  deleteDatabase() {
    return new Promise((resolve, reject) => {
      const isFile = stat(this._filepath)
  ...
  moveDatabase(filepath) {
    return new Promise((resolve, reject) => {
      if (typeof filepath !== 'string' || filepath === '') {
        reject(new Error('Filepath has to be a string and must be at least one character long'));
      }
      ...
      if (!isFile) {
        reject(new Error('Database to move is not a file.'));
      }

      copyFile(this._filepath, filepath)
        .then(() => {
          unlink(this._filepath)
            .then(() => {
              this._filepath = filepath;
  ...
  _loadDatabase() {
    return new Promise((resolve, reject) => {
      ...
      readFile(this._filepath, { encoding: 'utf-8' })
  ...
  _applyDatabase(db, restoreSettings) {
    ...
    if (Array.isArray(db.collections)) {
      this._collections = db.collections
        .map(collection => {
    ...
    if (restoreSettings === true) {
      Object.assign(this._settings, db.settings);
    }
  }
